...
final class PaymentMethodsAPITests: XCTestCase {
    ...
    func testGetPaymentMethod() async {
        ...
        do {
            ...
            let (thirdReceivedMethod, _) = try await PaymentMethodsAPI.getPaymentMethodWith(paymentMethodId: paymentMethod.id)
            XCTAssertNotNil(thirdReceivedMethod)
    ...
    func testGetCustomerPaymentMethods() async {
        ...
        do {
            session.data = try JSONEncoder().encode(PaymentMethodResponses.ListPaymentMethodsResponse(meta: nil, data: [paymentMethod, secondPaymentMethod]))
            let (thirdReceivedMethod, _) = try await PaymentMethodsAPI.getPaymentMethodsWithCustomer(customerId: paymentMethod.customer ?? "")
    ...
    func testCreateCardPaymentMethod() async {
        ...
        let secondReceivedMethod = try? await PaymentMethodsAPI.createCardPaymentMethod(request: request).0
        XCTAssertNil(secondReceivedMethod)
        ...
        do {
            session.data = try JSONEncoder().encode(paymentMethod)
            let (thirdReceivedMethod, _) = try await PaymentMethodsAPI.createCardPaymentMethod(request: request, encryptData: false)
            XCTAssertNotNil(thirdReceivedMethod)
            XCTAssertEqual(thirdReceivedMethod?.card, card)
            XCTAssertEqual(thirdReceivedMethod?.billing?.postalCode, "55555")
        } catch {
            XCTFail("Error should not be thrown")
        }
    }
    ...
    func testCreateACHPaymentMethod() async {
        ...
        paymentMethod.billing = billing
        paymentMethod.ach = account
        
        XCTAssertNotNil(paymentMethod.billing)
        XCTAssertNotNil(paymentMethod.ach)
        
        do {
            session.data = try JSONEncoder().encode(paymentMethod)
            let (thirdReceivedMethod, _) = try await PaymentMethodsAPI.createACHPaymentMethod(request: request)
            XCTAssertNotNil(thirdReceivedMethod)
        } catch {
            XCTFail("Error should not be thrown")
        }
    ...
    func testUpdatePaymentMethod() async {
        ...
        do {
            session.data = try JSONEncoder().encode(paymentMethod)
            let (thirdReceivedMethod, _) = try await PaymentMethodsAPI.updatePaymentMethodWith(paymentMethodId: paymentMethod.id, request: request)
            XCTAssertNotNil(thirdReceivedMethod)
            XCTAssertEqual(thirdReceivedMethod?.type, paymentMethod.type)
        } catch {
            XCTFail("Error should not be thrown")
    ...
    func testBlockPaymentMethodWithMethodId() async {
        ...
        do {
            session.data = try JSONEncoder().encode(paymentMethod)
            let (thirdReceivedMethod, _) = try await PaymentMethodsAPI.blockPaymentMethodWith(paymentMethodId: paymentMethod.id)
            XCTAssertNotNil(thirdReceivedMethod)
            XCTAssertEqual(thirdReceivedMethod?.type, paymentMethod.type)
    ...
    func testUnblockPaymentMethodWithMethodId() async {
        ...
        do {
            session.data = try JSONEncoder().encode(paymentMethod)
            let (thirdReceivedMethod, _) = try await PaymentMethodsAPI.unblockPaymentMethodWith(paymentMethodId: paymentMethod.id)
            XCTAssertNotNil(thirdReceivedMethod)
            XCTAssertEqual(thirdReceivedMethod?.type, paymentMethod.type)
            XCTAssertEqual(thirdReceivedMethod?.status, paymentMethod.status)
        } catch {
